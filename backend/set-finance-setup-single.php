...
include ('inc2.php');
    ...
    if($pp == 1){
        $query3p ="UPDATE sessioninfo set finsetup = 0 where sccode = '$sccode' and sessionyear LIKE '$sy%'";
	    $conn->query($query3p); 
    }
  

    $sql00xr = "SELECT * FROM financesetup where  sccode='$sccode' and sessionyear LIKE '$sy%' and id='$id'"; //echo $sql00xr;
    ...
    $sql00x = "SELECT * FROM sessioninfo where  sccode='$sccode' and sessionyear LIKE '$sy%' and finsetup = 0  order by stid LIMIT 1"; 
    ...
    if ($result00x->num_rows > 0) {while($row00x = $result00x->fetch_assoc()) {
        ...
        if($cls == 'Ten'){$amt = $ten;}
            ...
            $sql00xd = "SELECT * FROM stfinance where  sccode='$sccode' and sessionyear LIKE '$sy%' and  stid='$stid' and partid='$partid' ";
