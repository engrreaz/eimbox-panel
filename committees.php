...
$commlist = array();
$sql0 = "SELECT title FROM committee where sccode='$sccode' group by title order by title  ;";
$result00 = $conn->query($sql0);
if ($result00->num_rows > 0) {
    while ($row5 = $result00->fetch_assoc()) {
        $commlist[] = $row5;
    }
}
                    ...
                    <div class="table-responsive">
                        <table id="main-table-search" class="table table-bordered text-white"
                            ...
                            <tbody>
                                ...
                                $slx = 1;
                                $ooo = 0;
                                $sql0x = "SELECT * FROM committee where sccode='$sccode'  and (active_date<='$td' && expire_date >= '$td') order by  title;";
                                ...
                                if ($result0xd->num_rows > 0) {
                                    while ($row0x = $result0xd->fetch_assoc()) {
                                        ...
                                        $title = $row0x["title"];
                                        $cycle = $row0x["cycle"];
                                        $datefrom = $row0x["active_date"];
                                        $dateto = $row0x["expire_date"];
                                        $remaining = (strtotime($dateto) - strtotime($td)) / (3600 * 24);
                                        ?>
                                        <tr class="">
                                            <td><?php echo $slx; ?></td>
                                            ...
                                            <td>
                                                <div id="ssp<?php echo $id; ?>" class="btn-groupx" role="groupx"
                                                    ...
                                                    <button onclick="show(<?php echo $id; ?>,1);"
                                                        class="btn btn-inverse-success pt-2"><i
...
<script>
    ...
    function edit(id, taill) {
        window.location.href = 'bank-account.php?id=' + id;
    }
    function show(id, taill) {
        window.location.href = 'committee-details.php?id=' + id;
    }
